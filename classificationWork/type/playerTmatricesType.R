...
#Creation of transition matrix structure 
...
#Transition matrix calculated for each player in the dataset
for(i in 1:86){
  ...
  pVector
  playerMatrixdfType[i,]<-c(playerGamestyles$Player[i],playerGamestyles$`Player Gamestyle`[i],playerGamestyles$`Player Ranking`[i], pVector)
}

#Function to calculate transition matrices
playerStateMatrixType<-function(player){
  #filtering dataset to remove erroneous records
  ...
  #Relabeling column entries to group together shots and outcomes
  ...
  #-------------------------------------------SET UP----------------------------------------------------------
  #Get list of all possible state combinations, giving each state a number instead of using full state description
  statesIDs_type <- reducedOutcomes_type %>% group_by(`Shot Outcome`, `Shot Type`) %>% arrange(.by_group = TRUE) %>% 
    mutate(stateID=cur_group_id()) 
  
  #Group each shot to find sequential points using a point ID
  seqShotIDs_type <- reducedSts %>%  filter(`Shot Number`!= 'Serve') %>% 
    ...
    select(stateID, `Shot Index`, ovrID, `Shot By`) 
  
  #Self join previous table to find sequential shots within the same point
  ...
  #Tallies of transitions between states
  ...
  #-------------------------------------------SET UP----------------------------------------------------------
  #Calculate transtion probabilites
  ...
  #Add point end state and label states
